...
object SmartStreamSelector {
    ...
    fun rank(streams: List<StreamItem>, context: Context = Context()): List<StreamItem> =
        streams.withIndex()
            .sortedWith(compareByDescending<IndexedValue<StreamItem>> { score(it.value, context) }
                .thenBy { it.index })
            .map { it.value }
    ...
    private fun score(stream: StreamItem, context: Context): Int {
        ...
        if (resolution > 0) {
            score += when {
                ...
                context.displayHeight != null -> when {
                    resolution <= context.displayHeight!! -> 100 + resolution / 100
                    else -> maxOf(0, 100 - (resolution - context.displayHeight!!) / 20)
                }
        ...
        val bitrateKbps = context.estimatedBandwidthKbps
        if (bitrateKbps != null && bitrateKbps > 0 && sizeBytes != null) {
            // Prefer a conservative fit when network capacity is known.
            val durationSeconds = parsed?.duration?.takeIf { it > 0 }
            if (durationSeconds != null) {
                val requiredKbps = (sizeBytes * 8L / 1000L / durationSeconds).toInt()
                score += when {
                    requiredKbps <= bitrateKbps * 60 / 100 -> 35
                    requiredKbps <= bitrateKbps * 75 / 100 -> 20
                    requiredKbps <= bitrateKbps * 90 / 100 -> 5
                    requiredKbps <= bitrateKbps -> -30
                    else -> -100
                }
            }
        }
        ...
        if (context.preferredVideoCodec != null && codec == context.preferredVideoCodec.lowercase()) score += 15
        score += when (codec) {
        ...
        if (stream.clientResolve?.isCached == true) score += 20
    ...
    private fun resolutionHeight(value: String): Int {
        ...
        val p = Regex("(?:^|\\D)(2160|1440|1080|720|576|540|480|360)p?(?:\\D|$)").find(normalized)
        if (p != null) return p.groupValues[1].toInt()
        return when {
